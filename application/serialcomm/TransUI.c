...
#define TransUI_SendCmdFrame(p,pData,Dlen,Ctrl) TransUI_send(p,pData,Dlen,Ctrl,E_CMD_FRM)
...
static BOOL  TransUI_send           (STransUI *pTrans, uint8_t* pu8Data, uint16_t u16Dlen, uint8_t u8Ctrl, EPackageType bFrmType);
static void  TransUI_CheckTimeOutOfSendedFrame(STransUI *pTrans);
 ...
 *  @note
 */
void TransUI_RecvTask(STransUI *pTrans)
{
 ...
 *  @note
 */
void TransUI_SendTask(STransUI *pTrans)
{
    STransL2S   *pTransL2         = &pTrans->sTransL2;
    SMem        *pMem             = NULL;
    /*------------------------------------------------------------------------*/
    ...
    if(pTrans->sFlag.Bits.bUpdateWaitingACKFrameState == TRUE)
    {
        ...
        TransUI_CheckTimeOutOfSendedFrame(pTrans);
    }
    /*------------------------------------------------------------------------*/
 ...
 *  @note
 */
static BOOL TransUI_send(STransUI *pTrans, uint8_t* pu8Data,
 ...
 *  @note
 */
BOOL TransUI_Send(STransUI *pTrans, uint16_t u16Dlen, uint8_t* pu8Data,  uint8_t u8Ctrl)
{
    return TransUI_send(pTrans, pu8Data, u16Dlen, u8Ctrl, E_DATA_FRM);
}
/*****************************************************************************/
/** @brief
 *
...
{
	STransUI *pTrans = (STransUI *)pClbParam;
